...
def rbf_interpolation(data, downsample_framerate, final_framerate, phi, phi_inv, H):

    DS = final_framerate / downsample_framerate
    num_frames = data.shape[1]
    new_num_frames = int(num_frames * DS)

    F = torch.tensor(data.reshape(-1)).type(torch.float32).cuda()
    ...
    F_est = F_est.reshape(data.shape[0], new_num_frames).cpu()
    # F_est_imag = F_est_imag.reshape(data.shape[0], new_num_frames).cpu()

    return F_est.numpy()


if __name__ == "__main__":
    downsample_framerate = 500
    final_framerate = 1000
    DS = int(final_framerate / downsample_framerate)

    mats_path = "PALA_data_InVivoRatBrain/IQ"
    new_mats_path = f"PALA_GA_{downsample_framerate}Hz_ablation/IQ"
        ...
        if mat.endswith(".mat"):
            ...
            for i in tqdm(range(data_IQ.shape[0])):
                ...
                for frame in range(0, data_IQ.shape[2] - tmp + 1, tmp):
                    ...
                    grid = data_IQ[i, :, frame:end]

                    F_est = rbf_interpolation(grid, downsample_framerate, final_framerate, phi, phi_inv, H)
                    # total_frame_real.append(F_est_real)
                    # total_frame_imag.append(F_est_imag)
                    total_frame.append(F_est)

                total_frame = np.concatenate(total_frame, axis=1)
                # total_frame_imag = np.concatenate(total_frame_imag, axis=1)

                data_new.append(total_frame)
